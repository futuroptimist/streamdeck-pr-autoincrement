...
export function buildPrListFromUrl(url: string, count = 4): string {
	...
	if (!match) {
		throw new Error("Input does not match expected PR URL pattern");
	}

	const prNumber = Number.parseInt(match[1], 10);
	if (Number.isNaN(prNumber) || prNumber < 0 || !Number.isInteger(prNumber)) {
		throw new Error("Invalid PR number parsed from input");
	}
...
export class PRListAction extends SingletonAction {
	override async onKeyDown(ev: KeyDownEvent): Promise<void> {
		try {
			...
			streamDeck.logger.info("Successfully generated PR list from clipboard input");
		} catch (error) {
